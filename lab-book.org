...
capabilities to a currently running job or job step. One can attach to and
detach from jobs multiple times.
...
be used to send an arbitrary signal to all processes associated with a running
job or job step.
...
running jobs in priority order and then the pending jobs in priority order.
...
include things such as nodes going down or jobs approaching their time limit.
